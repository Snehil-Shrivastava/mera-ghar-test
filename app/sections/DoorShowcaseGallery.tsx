...
import { mera_door } from "@/lib/data";
import { ChevronLeft, ChevronRight, X } from "lucide-react";
import Image from "next/image";
import Link from "next/link";
import { useCallback, useEffect, useState } from "react";

const DoorShowcaseGallery = () => {
  const DoorGallery = mera_door;
    ...
    (direction: "prev" | "next") => {
      ...
      const newIndex =
        direction === "prev"
          ...
          : selectedImage.index + 1;
      if (newIndex < 0 || newIndex >= DoorGallery.length) return;
      // @ts-expect-error random
      setSelectedImage({ src: DoorGallery[newIndex].src, index: newIndex });
    },
    [selectedImage, DoorGallery],
  );
    ...
    <>
      ...
      {selectedImage && (
        ...
        >
          ...
          {/* Next button */}
          {selectedImage.index < DoorGallery.length - 1 && (
            <button
              onClick={(e) => {
                e.stopPropagation();
                navigate("next");
          ...
          >
            ...
            <p className="absolute -bottom-8 left-1/2 -translate-x-1/2 text-white/60 text-sm tabular-nums font-openSans">
              {selectedImage.index + 1} / {DoorGallery.length}
            </p>
          </div>
        </div>
      )}
...
export default DoorShowcaseGallery;
