    ...
    (direction: "prev" | "next") => {
      ...
      const newIndex =
        direction === "prev"
          ...
          : selectedImage.index + 1;
      if (newIndex < 0 || newIndex >= WardrobeGallery.length) return;
      // @ts-expect-error random
      setSelectedImage({ src: WardrobeGallery[newIndex].src, index: newIndex });
    },
    [selectedImage, WardrobeGallery],
  );
    ...
    <>
      ...
      {selectedImage && (
        ...
        >
          ...
          {/* Next button */}
          {selectedImage.index < WardrobeGallery.length - 1 && (
            <button
              onClick={(e) => {
                e.stopPropagation();
                navigate("next");
          ...
          >
            ...
            <p className="absolute -bottom-8 left-1/2 -translate-x-1/2 text-white/60 text-sm tabular-nums font-openSans">
              {selectedImage.index + 1} / {WardrobeGallery.length}
            </p>
          </div>
        </div>
      )}
